...
最后更新：2026-07-03 (分类功能已完善)

## 变更记录
- **2026-07-03**: 自动分类模块完整实现 — `classify_task` 改为幽灵卡提案模式，`_cmd_classify` 传递实际待处理任务上下文，幽灵卡支持 classify 类型，接受/拒绝回调完善。
- **2026-07-03**: 生成任务模块升级 — `_cmd_gen_tasks` 改为两阶段交互流程（AI 先询问需求再生成），弹窗显示进度状态，共享 session 保持对话上下文，后台轮询检测生成完成。
- **2026-07-03**: 自动排程模块完整实现 — `schedule_task` 改为幽灵卡提案模式（`ai_suggestions` 存储排程数据），`_cmd_schedule` 传递实际已分类任务上下文，幽灵卡支持 schedule 类型，接受/拒绝回调完善。
...
| # | 工具 | 状态 | 触发 | 离线降级 |
|---|------|------|------|---------|
| 1 | 生成大纲 | ✅ 基础可用 | 菜单/AI 面板 | 关键词推断 + 模板 |
| 2 | 生成任务 | ✅ 基础可用 | 菜单/AI 面板 | 提示配置 API Key |
| 3 | 自动分类 | ✅ 基础可用 | 菜单/AI 面板 | 列出任务 + 提示 |
| 4 | 自动排程 | ✅ 基础可用 | 菜单/AI 面板 | 列出任务 + 提示 |
...
- **分类/排程/验收的 Agent 调用未正确传递看板上下文** — ✅ 分类/排程已修复：`_cmd_classify` 和 `_cmd_schedule` 现在会拼接实际任务列表。验收仍有同样问题。
