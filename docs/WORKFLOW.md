...
```
待处理 ─→ 分类中 ─→ 已排程 ─→ 就绪 ─→ 执行中 ─→ 检查中 ─→ 已完成 ─→ 已归档
  │        ↑│        ↑│        ↑│       │  ↑      │  ↑
  │        ││        ││        ││       │  │      │  │
  └────────┘└────────┘└────────┘│       │  │      │  │
                                │       ├─待零件──┘  │
...
| `inspection` | 检查中 | 15 | 5 | ✓ | 维修后质量检查 |
| `parts_hold` | 待零件 | 10 | 6 | ✓ | 缺航材，等待到货 |
...
ALLOWED_TRANSITIONS = {
    ...
    "inspection":   ["completed", "in_progress"],   # 检查中 → 完成 / 返工
    "parts_hold":   ["ready", "scheduled"],         # 待零件 → 回到就绪 / 重新排程
...
| 就绪 → 检查中/已完成 | 跳过执行，没干活不能检查 |
| 执行中 → 已排程/就绪/待处理 | 已开工，不可向前回退 |
| 待零件 → 待处理/分类中/执行中 | 零件到货后回到就绪或排程，不跳过步骤 |
