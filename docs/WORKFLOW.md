...
```
待处理 ─→ 已分类 ─→ 已排程 ─→ 就绪 ─→ 执行中 ─→ 验收中 ─→ 已完成 ─→ 已归档
  │        ↑│        ↑│        ↑│       │  ↑      │  ↑
  │        ││        ││        ││       │  │      │  │
  └────────┘└────────┘└────────┘│       │  │      │  │
                                │       ├─阻塞中──┘  │
...
ALLOWED_TRANSITIONS = {
    ...
    "triage":       ["scheduled", "backlog"],       # 已分类 → 排程 / 退回待处理
