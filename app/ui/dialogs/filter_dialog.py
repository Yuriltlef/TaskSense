...
def open(page: ft.Page):
    ...
    def _apply(_):
        ...
        board_service.set_filters(f); dlg.close(); Toast.show(page, "筛选已应用", "info")

    def _clear(_):
        ata_dd.value = ""; pri_dd.value = ""; ata_dd.update(); pri_dd.update()
        board_service.set_filters(FilterState()); dlg.close(); Toast.show(page, "筛选已清除", "info")

    form = ft.Container(ft.Column([
        ft.Text("ATA 章节", size=s(12), color=theme.text_primary, font_family=ff, weight=ft.FontWeight.W_500), ata_dd,
        ft.Divider(height=s(14), color=ft.Colors.TRANSPARENT),
        ft.Text("优先级", size=s(12), color=theme.text_primary, font_family=ff, weight=ft.FontWeight.W_500), pri_dd,
    ...
    content = ft.Column([
        ...
        form, dlg_footer("清除", "应用筛选", _apply, on_cancel=_clear),
