...
def open(page: ft.Page, tid: str, col: str = None, index: int = -1):
    ...
    cur = t.priority.value if t else "cat_c"
    selected = {"val": cur if cur in [o[0] for o in options] else "cat_c"}
            ...
            content=ft.Column([
                ft.Row([ft.Icon(ft.Icons.FLAG_OUTLINED, size=s(14), color=color),
                        ft.Text(label, size=s(13), weight=ft.FontWeight.W_600,
                                color=color if sel else theme.text_primary, font_family=ff)], spacing=s(6)),
    ...
    def _select(v):
        ...
        for i, chip in enumerate(chips):
            s = options[i][0] == v; c = options[i][3]
            chip.border = ft.border.all(1.5, c if s else theme.border)
            chip.bgcolor = ft.Colors.with_opacity(0.06, c) if s else theme.card
            chip.content.controls[0].controls[1].color = c if s else theme.text_primary
            chip.update()

    def _confirm(_):
        p = selected["val"]
        try:
            task_service.update_task(tid, priority=Priority(p))
            if col is not None:
                task_service.move_task(tid, col, index=index)
                labels = {"aog":"AOG","cat_a":"Cat A","cat_b":"Cat B","cat_c":"Cat C","cat_d":"Cat D"}
                Toast.show(page, f"已分类 — {labels.get(p, p)}", "success")
            else: Toast.show(page, "优先级已更新", "success")
    ...
    form = ft.Container(ft.Column([
        ...
        ft.Row(chips[3:], spacing=s(8), alignment=ft.MainAxisAlignment.CENTER),
    ], spacing=s(8), tight=True), padding=ft.padding.all(s(14)))
    content = ft.Column([
        dlg_header(ft.Icons.FLAG_OUTLINED, "确认优先级", lambda e: dlg.close()),
        form, dlg_footer("取消", "确认", _confirm, on_cancel=lambda e: dlg.close()),
